...
def fit_plane_LSE_RANSAC_Optimizer(
    points, iters= 2, inlier_thresh=0.05, return_outlier_list=False
):
    # points: Nx4 homogeneous 3d points
    # return:
    ...
    plane_points = points
    plane_index = np.where(plane_points[:, 2] <= 0.5)[0]
    plane_points = plane_points[plane_index]
    ...
    for i in range(iters):
        ...
        chose_points = plane_points[chose_id, :]
    ...
    #final_points = points
    ground_index = np.where(final_points[:,2] <= 0.5)[0]
    final_points = final_points[ground_index]
    N = len(final_points)
    chose_id = np.random.choice(N, 50, replace=True)
    chose_points = points[chose_id, :]
    plane = fit_plane_LSE(chose_points)
...
def fit_plane_LSE_RANSAC(
    points, iters= 10, inlier_thresh=0.138, return_outlier_list=False
):
    # points: Nx4 homogeneous 3d points
    # return:
    #   plane: 1d array of four elements [a, b, c, d] of ax+by+cz+d = 0
...
def create_pcd(points):
    ...
    pcd.points = open3d.Vector3dVector(points[:, :3])
    return pcd

def get_fit_plane_LSE_RANSAC(points):
    start = time.clock()
    ...
    for p in [p1]:
        ...
        non_ground_index = np.where(points[:, 2] >= zz)[0]
        ground_index = np.where(points[:, 2] < zz)[0]
        points_up = points[np.where(points[:, 2] >= zz)]
        points_down = points[np.where(points[:, 2] < zz)]
    elapsed = (time.clock() - start)
    #print(elapsed)
    return points_up, points_down, non_ground_index, ground_index, elapsed


def get_fit_plane_LSE_RANSAC_Opitimizer(points):
    start = time.clock()
    points = np.concatenate((points[:, :3], np.ones((points.shape[0], 1))), axis=1)
    ...
    p_set_1 = points
    p1, inlier_list1, outlier_list1 = fit_plane_LSE_RANSAC_Optimizer(
        p_set_1, return_outlier_list=True
    )
    ps = [p1]
    for p in [p1]:
...
if __name__ == "__main__":
    ...
    points = np.load('/media/data1/vo/sequences_0.06/08/velodyne/000000.npy')
    points = np.concatenate((points[:, :3], np.ones((points.shape[0], 1))), axis=1)
    ...
    for p in [p1]:
       ...
       # points_up.tofile('/home/kid/Desktop/000000.bin')
      #  print('Point after filtering ',points_up.shape)

       # ptup = create_pcd(points_up)
        #ptdn = create_pcd(points_down)
        #ptup.paint_uniform_color([0.5, 0.5, 0.5])
       # open3d.draw_geometries([ptup, ptdn])
